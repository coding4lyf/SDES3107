// Part2 ["I hate Coding"] - Project 1 - Alexandra Hall - Z3375459
// 
...
void setup() {
  // I CHANGED THE SIZE
  size(550, 550); //size should be multiple of img width and height
  smooth();
...
void draw() {
  ...
  for (int gridX = 0; gridX < img.width; gridX++) {
    for (int gridY = 0; gridY < img.height; gridY++) {
      ...
      case 4:
        ...
        stroke(0);
        float w4 = map(greyscale, 10, 200, 10, 0);
        strokeWeight(w4 * mouseXFactor + 10);
  ...
  // I ADDED TEXT, CHANGED THE FONT, ALLIGNED IT, CREATED 
  //A FUNCTION SO THE COLORMODE CHNAGES WITH mouseX & mouseY

  textSize(100);
  loadFont ("Impact-48.vlw");
  colorMode(HSB, width, height, 100);
  // CHANGE THE BEHAVIOUR SO TEXT MOVES HORIZONTAL WITH MOUSE. 
  fill(mouseX, mouseY, 100);
  text("I", mouseX - 10, 100);
  textSize(200);
  text("HATE", mouseX - 250, 300);
  textSize(80);
  text("CODING", mouseX - 150, 450);
  colorMode(RGB, 255, 255, 255);
  colorMode(RGB, 255, 255, 255);
}

//CHANGING THE FUNCTION --> IMAGE STYLE CHANGES WHEN MOUSE IS CLICKED.
